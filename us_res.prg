...
FUNCTION MAIN( ... )
   ...
   Local Version := "01.06", bLoop := .T., cLinea := "", cFines := { Chr(13) + Chr(10), Chr(10) }, hFiIn, cont := 0, hFiOut
   ...
   If bInclude
      If File( cFileIn )
         US_FileChar26Zap( cFileIn )    // Main RC
         hFiIn := FOpen( cFileIn )
         If FError() != 0
         ...
         EndIf
      Else
         QPM_Log( "US_Res 555E: Error opening Input File " + cFileIn )
         QPM_Log( "" + HB_OsNewLine() )
         ERRORLEVEL( 1 )
         Return 8
      EndIf
   ...
   If FError() = 0
      ...
      Do While bLoop
         ...
         If FWrite( hFiOut, cLinea, nBytesSalida ) < nBytesSalida
            QPM_Log( "US_Res 012E: Error saving into Out File (" + AllTrim( Str( FError() ) ) + ") " + cFileOut )
            FClose( hFiIn )
            QPM_Log( "" + HB_OsNewLine() )
            ERRORLEVEL( 1 )
            Return 16
