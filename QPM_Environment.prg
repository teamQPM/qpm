...
Function LoadEnvironment
   // LoadEnvironment2
   Local EnvironmentMemo , EnvironmentVersion:=0 , EnvironmentMemoAux
   Local LOC_cLine , nInx := 0
   PRIVATE vConfig := {}
   Gbl_TEditor := ''
   //
   ...
   if PUB_bLite .and. file( US_FileNameOnlyPathAndName( PUB_cProjectFile )+ ".cfg" )
      ...
      if empty( PUB_MigrateFolderFrom )
         PUB_MigrateFolderFrom := GetWindowsFolder() // voy al metodo viejo
      endif
   endif
...
#include "QPM_CompatibilityOpenGlobal.CH"
   ...
   For i := 1 To MLCount ( EnvironmentMemo , 254 )
      ...
      //     VentanaMain.CSearch.AddItem( US_WordSubStr( LOC_cLine , 2 ) )
...
Function SaveEnvironment
   ...
   Next i
   //
   For i:=1 to len( vLastOpen )             // to 1 step -1
      DO EVENTS
